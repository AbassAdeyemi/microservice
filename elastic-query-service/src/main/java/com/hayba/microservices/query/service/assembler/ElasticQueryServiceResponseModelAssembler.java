...
public class ElasticQueryServiceResponseModelAssembler extends
    ...
    public ElasticQueryServiceResponseModel toModel(TwitterIndexModel twitterIndexModel) {
        ElasticQueryServiceResponseModel responseModel = elasticToResponseModelTransformer
                .getResponseModel(twitterIndexModel);

        responseModel.add(linkTo(methodOn(ElasticDocumentController.class).
                getDocumentById(responseModel.getId())).withSelfRel());

        responseModel.add(linkTo(methodOn(ElasticDocumentController.class)
        .getDocuments()).withRel("documents"));
